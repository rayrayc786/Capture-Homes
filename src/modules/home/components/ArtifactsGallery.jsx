    ...
    <section id="gallery" className="py-32 px-6 md:px-12 bg-surface">
      <div className="container mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-20 gap-8">
          <div>
            <span className="text-primary font-bold tracking-widest text-xs uppercase">{data.eyebrow}</span>
            <h2 className="text-4xl md:text-5xl font-extrabold tracking-tighter mt-4 text-on-surface">{data.title}</h2>
          </div>
          <a className="text-sm font-bold border-b-2 border-on-surface pb-1 tracking-widest uppercase hover:text-primary hover:border-primary transition-all transition-colors" href="#">View All Work</a>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:auto-rows-[450px]">
          {data.items.map((item) => (
